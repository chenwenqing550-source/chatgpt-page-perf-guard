(() => {
  ...
  function buildPrompt() {
    return [
      ...
      "- VERIFIED：已经有 Evidence 支持的事实/测试/结果。",
      "- PENDING：尚未验证或仍待完成。",
      "- BLOCKED：真实阻塞及解除条件。",
      "- HISTORICAL：仅供追溯的旧版本/旧方案，不得作为当前基线。",
      "- REJECTED：已否决/已废弃方案及原因。",
      "",
  ...
  function dispatchInput(node, prompt) {
  ...
  function insertPrompt(prompt = buildPrompt()) {
    ...
    if (!composer) {
      return { ok: false, error: "未找到 ChatGPT 输入框" };
    }
    dispatchInput(composer, String(prompt));
    return { ok: true };
  }

  globalThis.CGPTHandoff = Object.freeze({ buildPrompt, insertPrompt });
